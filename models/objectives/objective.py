# models/objectives/objective.py
"""
...
# Import libraries
from numpy import array, cos, exp, float64, median, pi, prod, repeat, sin, sqrt
import numpy as np

# Import model classes
from models.objectives.options import NodeWeight, DegreeWeight
    

class Objective(object):
    
    def __init__(self, opts, num_vars, kk):
        '''Initialize an objective function with the specified options.'''
        self.set_main_vars(num_vars, kk)
    ...
    def set_main_vars(self, num_vars, kk):
        '''Set main variables used for most objectives.'''
        self.set_vars_per_node(num_vars)
        self.set_degree(kk)
        self.set_total_vars()
    ...
    def set_weight(self, opts, divisor=1):
        ...
        if opts['weight'] == 'node':
            self.norm_weight = NodeWeight(self.nn, self.kk, divisor)
        elif opts['weight'] == 'degree':
            self.norm_weight = DegreeWeight(self.nn, self.kk, divisor)
        else: raise RuntimeError('Weight ' + opts['weight'] \
                                 + ' for ' + self.__class__.__name__ \
                                 + ' function is invalid.')
            
    def set_vars_per_node(self, num_vars):
        '''Set the number of variables that each node (agent) owns.'''
        self.num_vars = num_vars
    
    def set_degree(self, kk):
        '''Sets the degree of each node (kk).'''
        self.kk = repeat(array(kk), self.num_vars)
        
    def set_total_vars(self):
        '''Set total number of variables (nn) based on length of repeated 
        vector of degrees.'''
        self.nn = len(self.kk)
    ...
    def calc_peaks(self):
        ...
        # If frequency is degree-based, every dimension has diff num of peaks
            ...
            peaks = prod(self.kk.astype(float) + 1)
    ...
    def get_frequency(self):
        ...
        if not(hasattr(self, 'omega')):
            if self.frequency == 'uniform':
                ...
                raise RuntimeError('Frequency ' + self.frequency \
                                 + ' for ' + self.__class__.__name__ \
                                 + ' function is invalid.')
...
class Average(Objective):
    ...
    def __init__(self, opts, num_vars, kk):
        super().__init__(opts, num_vars, kk)
...
class Root(Objective):
    ...
    def __call__(self, xx):
        super().__call__(xx)
        return self.norm_sum(safe_sqrt(self.xx))
...
class Sin2sphere(Objective):
    ...
    def __init__(self, opts, num_vars, kk):
        ...
        # Set the numbers of variables and degrees
        self.set_main_vars(num_vars, kk)
...
class Sin2root(Objective):
    ...
    def __call__(self, xx):
        super().__call__(xx)
        self.get_frequency()        
        return self.norm_sum(sin(self.omega*self.xx)**2 + safe_sqrt(self.xx))
...
class Ackley(Objective):
    ...
    def __call__(self, xx):
        ...
        xx_term = sum(self.xx**2)
        cos_term = sum(cos(self.cc*self.xx))
...
def safe_sqrt(x):
    """
    Compute sqrt while handling numerical underflow gracefully.
    
    This is used instead of direct sqrt() to handle floating point precision 
    effects that could result in tiny negative values (~1e-8) that are 
    effectively zero but would cause sqrt to complain.
    """
    return sqrt(np.maximum(x, 0))
